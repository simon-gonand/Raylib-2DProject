...
Renderer2DComponent::Renderer2DComponent(std::shared_ptr<Actor> Owner, const char* TexturePath, bool bAutoActivate,
	...
	:RendererComponent(Owner, bAutoActivate, InLocation, InRotation, InScale, InAnimManager)
{
...
void Renderer2DComponent::Update(float DeltaTime)
{
	...
	if (DefaultTexture2D.width > 0.0f && DefaultTexture2D.height > 0.0f)
	{
		Rectangle source = {
			0,
			0,
			DrawRotation.y < 0.0f ? -DefaultTexture2D.width : DefaultTexture2D.width,
	...
	else 
	{
		...
		Rectangle rect = {
			DrawLocation.x,
			DrawLocation.y,
			DrawRotation.y < 0.0f ? -ScaleSize.x : ScaleSize.x,
			ScaleSize.y
		};
